...
def memory_hierarchy_dut(imc_array, visualize=False):
    ...
    sram_local_iBuf = MemoryInstance(
        name="Input_buffer",
    ...
    ac_time, sram_area, sram_r_cost, sram_w_cost = get_cacti_cost(cacti_path, tech_param["tech_node"], "sram",
                                                                  sram_size, sram_bw,
                                                                  hd_hash=str(hash((sram_size, sram_bw, random.randbytes(8)))))
    sram_local_oBuf = MemoryInstance(
        name="Output_buffer",
        size=sram_size * 8, # byte -> bit
        r_bw=sram_bw,
        w_bw=sram_bw,
    ...
    ##################################### on-chip global memory hierarchy building blocks #####################################
    ...
    ac_time, sram_area, sram_r_cost, sram_w_cost = get_cacti_cost(cacti_path, tech_param["tech_node"], "sram",
                                                                  sram_size, sram_bw,
                                                                  hd_hash=str(hash((sram_size, sram_bw, random.randbytes(8)))))
    sram_1024KB_256_3r_3w = MemoryInstance(
        name="Global_buffer",
        size=sram_size * 8, # byte -> bit
        r_bw=sram_bw,
        w_bw=sram_bw,
    ...
    dram_ac_cost_per_bit = 7.91 # unit: pJ/bit
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=sram_local_iBuf,
        operands=("I1",),
        port_alloc=(
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=sram_local_oBuf,
        operands=("O",),
        port_alloc=(
            {"fh": "rw_port_1", "tl": "rw_port_1", "fl": "rw_port_1", "th": "rw_port_1"},
        ),
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=sram_1024KB_256_3r_3w,
        operands=("I1","I2","O",),
        port_alloc=(
            {"fh": "rw_port_1", "tl": "rw_port_1", "fl": None, "th": None},
            {"fh": "rw_port_1", "tl": "rw_port_1", "fl": None, "th": None},
...
def imc_array_dut():
    ...
    hd_param = {
        ...
        "group_depth":          8,          # group depth in each PE
    ...
    dimensions = {
        "D1": 16,    # wordline dimension   # 横
        "D2": 32,   # bitline dimension       # 竖
...
def cores_dut():
    ...
    memory_hierarchy1 = memory_hierarchy_dut(imc_array1, visualize=True)

    core = Core(0, imc_array1, memory_hierarchy1)
