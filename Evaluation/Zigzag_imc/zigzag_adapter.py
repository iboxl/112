...
def loopdim_to_zigzag_workload(
...
def run_zigzag_mapper_for_layer(
    ...
    if use_cache and cme_path.is_file():
        with open(cme_path, "rb") as fh:
            return pickle.load(fh)

    accelerator = to_zigzag_accelerator(spec, acc_name="CIM_ACC_TEMPLATE")
    workload = loopdim_to_zigzag_workload(loopdim, layer_id=0, spec=spec)
    mapping = {
        "default": {
            "core_allocation": 0,
            "memory_operand_links": {"O": "O", "W": "I2", "I": "I1"},
            "spatial_mapping_hint": {"D1": ["K"], "D2": ["FX", "FY", "C"], "D3": ["G", "K", "OX", "OY"]},
        }
    }
...
# ── Phase 3: B2 — Force Q-spatial on ZigZag-IMC ─────────────────────────────
#
# CRITICAL NAMING: ZigZag uses "OX" for MIREDO's "Q" dimension.
# D3 = cores (size 8) maps to the outer spatial axis.
# Passing spatial_mapping = {"D3": ("OX", N)} forces OX=N onto D3;
# D1 and D2 are left free for SpatialMappingGeneratorStage to fill.
#
# These functions are ADDITIVE — they do not modify the baseline path.
# Cache is stored under a distinct subdir ("force_q_spatial") so it never
# collides with existing cme.pkl files from the baseline path.


def _force_q_spatial_cache_root(architecture: str, objective: str, spec, q_size: int) -> Path:
    """Separate cache root for force_q_spatial runs to avoid baseline collisions."""
    from Evaluation.common.EvalCommon import repo_root
    fp = _spec_fingerprint_inline(spec) if spec is not None else "legacy"
    base = repo_root() / "Evaluation" / "Zigzag_imc" / "output"
    dirname = f"{objective.lower()}_{architecture}_{fp}_force_q_spatial_ox{q_size}"
    return base / dirname


def run_zigzag_mapper_for_layer_force_q_spatial(
    spec,
    loopdim: Dict[str, int],
    objective: str,
    work_dir: Path,
    ...
    """
    Run ZigZag mapper with D3 forced to ("OX", Q) where Q = loopdim["Q"].

    The partial spatial_mapping {"D3": ("OX", Q)} is injected into mapping["default"].
    SpatialMappingGeneratorStage sees len(user_spatial_mapping) == 1 < len(oa_dims) == 3,
    so it calls UserSpatialMappingGenerator(defined_mapping={"D3": ("OX", Q)}) which
    fixes D3 while freely generating D1 and D2.

    The resulting cme.pkl is saved under a distinct cache root so it never overwrites
    the baseline cme.pkl produced by run_zigzag_mapper_for_layer().
    """
    from Evaluation.common.EvalCommon import objective_to_opt_flag

    q_size = int(loopdim["Q"])
    work_dir = Path(work_dir)
    cme_path = work_dir / "cme.pkl"
    if use_cache and cme_path.is_file():
        with open(cme_path, "rb") as fh:
    ...
    # Force OX=Q onto D3 (cores axis).  D1 and D2 are generated by ZigZag.
    # NOTE: ZigZag automatically scales down the unrolling size if it exceeds
    # the layer dimension size, so OX=7 is safe for L2 (Q=7).
    mapping = {
        "default": {
            "core_allocation": 0,
            "memory_operand_links": {"O": "O", "W": "I2", "I": "I1"},
            ...
            # Partial spatial_mapping: fix D3 to OX=q_size; D1/D2 generated freely.
            "spatial_mapping": {"D3": ("OX", q_size)},
        }
    }
...
def run_for_layer_force_q_spatial(
    acc, ops, loopdim, model_name, architecture, objective, use_cache: bool = True
):
    """
    Variant of run_for_layer() that forces Q into the spatial (D3/cores) axis.

    Intended for Phase 3 / EXP-2c B2 mechanism experiment.
    Uses a separate cache namespace (_force_q_spatial_cache_root) to avoid
    overwriting baseline cme.pkl files.
    """
    ...
    if spec is None:
        raise RuntimeError(
            f"zigzag_adapter.run_for_layer_force_q_spatial: no HardwareSpec resolved for "
            f"architecture={architecture!r}; cannot build ZigZag accelerator."
        )

    q_size = int(loopdim["Q"])
    cache_root = _force_q_spatial_cache_root(architecture, objective, spec, q_size)
    work_dir = cache_root / "per_layer" / loopdim_fingerprint(loopdim)

    import time as _time
    _mapper_t0 = _time.time()
    cme = run_zigzag_mapper_for_layer_force_q_spatial(
        spec=spec,
        loopdim=loopdim,
        objective=objective,
        work_dir=work_dir,
    ...
    return BaselineRunResult(
        ...
        dataflow=loops,
        metadata={
            "policy": "zigzag_imc_force_q_spatial_ox_on_d3",
            "force_q_spatial": True,
            "ox_forced_size": q_size,
            "model": model_name,
