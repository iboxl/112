...
def memory_hierarchy_dut(imc_array, visualize=False):
    ...
    sram_local_iBuf_256KB = MemoryInstance(
        name="sram_local_iBuf",
    ...
    ac_time, sram_area, sram_r_cost, sram_w_cost = get_cacti_cost(cacti_path, tech_param["tech_node"], "sram",
                                                                  sram_size, sram_bw,
                                                                  hd_hash=str(hash((sram_size, sram_bw, random.randbytes(8)))))
    sram_local_oBuf_256KB = MemoryInstance(
        name="sram_local_oBuf",
        size=sram_size * 8, # byte -> bit
        r_bw=sram_bw,
        w_bw=sram_bw,
        r_cost=sram_r_cost,
    ...
    sram_size = 1024 * 1024 * 1024 # unit: byte
    sram_bw = 1024
    ac_time, sram_area, sram_r_cost, sram_w_cost = get_cacti_cost(cacti_path, tech_param["tech_node"], "sram",
                                                                  sram_size, sram_bw,
                                                                  hd_hash=str(hash((sram_size, sram_bw, random.randbytes(8)))))
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=sram_local_iBuf_256KB,
        operands=("I1",),
        port_alloc=(
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=sram_local_oBuf_256KB,
        operands=("O",),
        port_alloc=(
            {"fh": "w_port_1", "tl": "r_port_1", "fl": "w_port_2", "th": "r_port_2"},
        ),
...
def imc_array_dut():
    ...
    dimensions = {
        "D1": 32,    # wordline dimension
        "D2": 64,   # bitline dimension
