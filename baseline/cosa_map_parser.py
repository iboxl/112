...
def _factors_to_loop_tuples(factors: dict[str, int], permutation: str) -> list[tuple[str, int]]:
    ...
    for dim in permutation:
        ...
        if dim == "N":
            if val > 1:
                raise ValueError("MIREDO CoSA baseline replay currently requires N=1")
            continue
        if val > 1:
...
def _infer_loop_dim(ir: CoSAMapIR) -> dict[str, int]:
    ...
    if totals["N"] != 1:
        raise ValueError(
            f"MIREDO CoSA baseline replay currently requires N=1, got N={totals['N']} in {ir.source_path}"
        )
    return {
        "R": totals["R"],
        "S": totals["S"],
