...
def __main__(**kwargs):
    ...
    # leakage_zz, dynamic_zz, leakage_mi, dynamic_mi = 0, 0 ,0, 0 
    ...
    for i, (Conv, loopdim) in enumerate(zip(convs, loopdims)):
        ...
        Logger.info('\n\n'+'* '*20+f"Layer {i}"+' *'*20)
        ops = WorkLoad(cfg, loopDim=loopdim)
        Logger.info(ops)
        ...
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -#   
            ...
            for dChar in ['P','Q','H','W']: #newdim[dChar] += (loopdim[dChar] % 2)
                if loopdim[dChar] % 2==1 and loopdim[dChar]>15:
                    newdim[dChar] += 1
            l_solver, e_solver, edp_solver, l_simu, e_simu, PD_M = SolveMapping(acc=accelerator, ops=WorkLoad(cfg, loopDim=newdim), outputdir=outputdir_layer)
